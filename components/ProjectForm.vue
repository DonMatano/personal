...
<template>
  <form class="flex flex-col gap-4">
    ...
    <textarea
      rows="4"
      maxlength="250"
      placeholder="Quick project description"
      class="bg-transparent border-b border-white py-3 px-4 outline-none focus:border-accent-teal"
    ...
    <div class="flex flex-col">
      <Editor
        ...
        can-edit
        @dataSaved="bodyContentSaved"
      />
    </div>
    <ImagePreview
      v-if="projectCoverPageURL"
      :image-u-r-l="projectCoverPageURL"
      image-caption="Cover"
    ...
    <ImageUploader
      :show-modal="isShowingUploadCoverPageModal"
    ...
    <div class="flex items-center">
      ...
      >
        ...
        <option v-for="tag in tags" :value="tag.id" class="uppercase">
          {{ tag.name }}
        </option>
      </select>

      <button
        v-if="!isShowingAddTechForm"
        type="button"
    ...
    <div class="flex flex-wrap gap-2">
      <TechItem
        ...
        :key="tag.id"
        :tech="tag"
        @delete="(id) => emit('delete_selected_tech', id)"
      />
    </div>
    <div class="flex flex-wrap">
      <ImagePreview
        v-for="fileURL in projectImagesURLs"
        :key="fileURL"
        :image-u-r-l="fileURL"
    ...
    <button
      type="button"
      class="justify-end px-4 py-1 border border-accent-teal"
      @click="isShowingUploadProjectFilesModal = true"
    >
      Upload project files
    </button>
    <ImageUploader
      multiple
      :show-modal="isShowingUploadProjectFilesModal"
    ...
    <label for="publish">
      ...
      <input
        ...
        type="checkbox"
        class="ml-2"
        @change="(e) => emit('publish_updated', e.target.checked)"
      />
    </label>
    <button
      type="button"
